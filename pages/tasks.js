...
export default function TasksPage(props) {
  ...
  const [
    ...
    fetchWorkflow,
  ] = useAxios(getWorkflow(props.currentDatasetId), { manual: true });
  const [
    {
      data: workflowTasks,
    ...
    const taskList = (
      <ListGroup>
        {milestone.tasks.map((task) => (
          ...
          >
            <CheckboxWithLabel checked={task.completed} label={task.title} />
          </ListGroup.Item>
        ))}
      </ListGroup>
    );
    ...
    function Body() {
      if (fetchWorkflowError) {
        ...
        return (
          <FetchWorkflowTasksError
            error={{
              title: "workflowTasksError",
              data: fetchWorkflowTasksError,
            }}
          />
        );
      } else if (loading) {
