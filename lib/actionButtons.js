...
const taskNotReachedTooltip = [
  t`You are currently previewing a future task.`,
  t`Click "What's Next?" to see your latest actionable task.`,
];
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [
          t`This workflow is now complete.`,
      ...
      if (manual) {
        ...
        return {
          enabled: false,
          tooltip: [
            t`Task status is automatically checked by system.`,
            t`Click "What's Next?" once task is complete.`
          ]
        };
      }
    }
...
export function priorTaskButton(workflowState) {
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [t`This is your first task.`]
      }
    } else {
      return { enabled: true }
    }
...
export function nextTaskButton(workflowState) {
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [t`This workflow is now complete.`]
      };
    } else if (isLatestTask) {
      return {
        enabled: false,
        tooltip: [
          t`This is your current task.`,
          t`Click "What's Next?" to proceed.`
        ]
      };
    } else {
      return { enabled: true };
