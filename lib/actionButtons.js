...
const taskNotReachedTooltip = [
  <Trans id="You are currently previewing a future task." />,
  <Trans id="Click “What's Next?” to see your latest actionable task." />,
];
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [
          <Trans id="This workflow is now complete." />,
          <Trans id="No further action is required." />
        ]
      };
      ...
      if (manual) {
        ...
        return {
          enabled: false,
          tooltip: [
            <Trans id="Task status is automatically checked by system." />,
            <Trans id="Click “What's Next?” once task is complete." />
          ]
        };
      }
    }
...
export function priorTaskButton(workflowState) {
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [<Trans id="This is your first task." />]
      }
    } else {
      return { enabled: true }
    }
...
export function nextTaskButton(workflowState) {
  ...
  const { enabled, tooltip } = (() => {
    if (!reached) {
      ...
      return {
        enabled: false,
        tooltip: [<Trans id="This workflow is now complete." />]
      };
    } else if (isLatestTask) {
      return {
        enabled: false,
        tooltip: [
          <Trans id="This is your current task." />,
          <Trans id="Click “What's Next?” to proceed." />
        ]
      };
    } else {
      return { enabled: true };
